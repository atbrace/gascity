...
func TestSessionDrainedPastPollerGraceFallsBackToUpdatedAt(t *testing.T) {
	cr := &CityRuntime{sessionDrains: newDrainTracker()}
	now := time.Now()

	mk := func(state, sleepReason string, updatedAt time.Time) beads.Bead {
		return beads.Bead{
			Metadata:  map[string]string{"state": state, "sleep_reason": sleepReason},
			UpdatedAt: updatedAt,
		}
	}
	cases := []struct {
	...
	// In-memory drain clock takes precedence over UpdatedAt.
	...
	if cr.sessionDrainedPastPollerGrace(info, now) {
		t.Errorf("fresh in-memory drain clock must outrank stale UpdatedAt")
	}
