...
| RUNTIME-SEL-014 | Hybrid remote arm is selectable | `hybrid` composes a local tmux arm with a remote arm named by `[session] hybrid_remote` — any registered selection name, builtin or pack-declared (`[runtimes.<name>]`), so one city can route match-selected sessions to a cluster runtime while the rest stay local. Empty defaults to `k8s`, the previously hardcoded arm. Resolution is strict (`Registry.NewStrict`): an unregistered name is a construction error naming it, never the tmux fallback of RUNTIME-SEL-006, because a remote arm that silently became local would run remote-matched sessions on the control-plane host with no signal. `hybrid_remote = "hybrid"` is rejected rather than nested. Because `Clone` copies factories by value, `runtimeRegistryForCity` rebinds `hybrid` on the clone (`Registry.Rebind`) so its remote arm resolves against that clone's pack runtimes and the process-global registry stays unmutated. | `internal/config/config.go` (`SessionConfig.HybridRemote`); `cmd/gc/providers.go` (`newHybridProvider`, `hybridRemoteName`); `cmd/gc/runtime_registry.go` (`hybridFactory`, `runtimeRegistryForCity`); `internal/runtime/registry/registry.go` (`NewStrict`, `Rebind`); `cmd/gc/hybrid_provider_test.go`; `internal/runtime/registry/registry_test.go` `TestNewStrict*`, `TestRebind*` |
