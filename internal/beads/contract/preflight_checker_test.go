...
// TestPreflightDefersIdentityToNativeOpenForExternalEndpoint covers external
// endpoints the direct project_id SQL probe (managedDoltOpenDatabase) cannot
// authenticate — e.g. a hosted beads-gateway (EIA-as-username + TLS) or a
// self-hosted authed dolt sql-server whose credentials live in the doltauth
// chain rather than GC_DOLT_PASSWORD. For an external endpoint the
// authoritative database _project_id is verified by beadslib at native-open
// time (verifyProjectIdentity over the authenticated connection), so the
// identity check defers to that gate and keeps the scope native-eligible
// instead of degrading to the shell BdStore — without claiming a
// control-plane confirmation it cannot make.
func TestPreflightDefersIdentityToNativeOpenForExternalEndpoint(t *testing.T) {
	...
	// Direct DB probe fails to authenticate the external endpoint...
	checker.DatabaseProjectID = func(string) (string, bool, error) {
		return "", false, errors.New("dial external endpoint: access denied")
	}
