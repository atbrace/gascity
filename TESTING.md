...
| Audit baseline | all tracked test source | subprocess: 664 calls / 193 files (historical regex census: 495 / 135) | ga-cp3hwi | tracked test source totals remain visible as audit evidence; ga-cp3hwi owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Medium owner | `internal/hooks` package `hooks` | TestPiIdleDrainRetriesRejectedFollowUpWithoutRedraining: subprocess | sys-xtzg5w | the Pi idle-redrive provider-boundary regression is a checked Medium subprocess owner; the one Node subprocess is confined to TestPiIdleDrainRetriesRejectedFollowUpWithoutRedraining, which executes the real bundled extension against a deterministic mocked Pi API and fake gc binary | P0.4b | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | subprocess: 451 calls / 131 files (historical regex census: 380 / 98) | ga-cp3hwi | untagged subprocess call/file totals cannot grow; reductions must lower this baseline; each process-owning test removes or replaces its source call site | D1/D2/D5/D6/E6 | 2026-10-01 |
