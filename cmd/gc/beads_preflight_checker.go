...
// external Dolt endpoint (e.g. a hosted beads-gateway or a self-hosted authed
// dolt sql-server). The direct project_id probe cannot authenticate such
// endpoints with control-plane credentials alone, so when it comes back
// unconfirmed the identity check defers to beadslib's native-open verification
// (which authenticates via the projected credentials and refuses to connect on
// a _project_id mismatch) instead of degrading the scope off the native store.
