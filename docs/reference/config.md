...
| `remote_match` | string |  |  | RemoteMatch is a substring pattern for the hybrid provider to route sessions to the remote backend. Sessions whose names contain this pattern go remote; all others stay local (tmux). Overridden by the GC_HYBRID_REMOTE_MATCH env var if set. |
| `hybrid_remote` | string |  | `k8s` | HybridRemote names the runtime the hybrid provider uses as its remote backend — any registered selection name, including a pack-declared [runtimes.&lt;name&gt;] runtime. Empty defaults to "k8s", the historical hardcoded backend. Unlike the city-wide provider name, an unregistered value here is a construction error rather than the tmux fallback: a remote arm that silently became local tmux would run remote-matched sessions on the control-plane host with no signal. |
